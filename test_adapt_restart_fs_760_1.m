% test_adapt_restart_fs_760_1.m
% Tests various sGMRES adaptive restart strategies using the fs_760_1.m matrix.
...
nrestarts = 20; % max number of restarts
m = 30;         % number of Arnoldi iterations
maxiters = nrestarts*m; % max number of total iterations

t = 2;         % Arnoldi truncation parameter
tol = 1e-15;   % convergence tolerance

rng(1);
s = 2*(m+1);       % sketching dimension
...
%% sGMRES with truncated Arnoldi, restart when basis gets too large
x0 = zeros(n,1);
r0 = b;
...
nexttile
semilogy(berr2,'LineWidth',6); hold on;
