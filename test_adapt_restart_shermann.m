% test_adapt_restart_shermann
% Tests various sGMRES adaptive restart strategies using the shermann matrix.
...
m = 200;       % number of Arnoldi iterations
t = 2;         % Arnoldi truncation parameter
tol = 1e-15;   % convergence parameter
...
%% sGMRES with truncated Arnoldi, restart when basis condition gets too large
x0 = zeros(n,1);
r0 = b;
...
semilogy(berr1, 'LineWidth',3);
semilogy(berr4, 'LineWidth',3);
semilogy(berr6,'LineWidth',3 );
semilogy(berr5,'LineWidth',3 , 'LineStyle','--');

xlabel("iteration",'interpreter','latex');
ylabel("backward error", 'interpreter','latex');
ylim([1e-15 1e0])
yticks([1e-15 1e-10 1e-5 1e0]);
lgd = legend('sGMRES (monitor $\tau_{i}$)', 'sGMRES (monitor $\kappa(B_{1:i})$)','sGMRES (adapt monitor $\tau_{i}$)', 'sGMRES (adapt monitor $\kappa(B_{1:i})$)', 'sGMRES (adapt monitor $\tau_{i}$ and $\kappa(B_{1:i}))$',  'GMRES', 'Location','southoutside','interpreter','latex');
